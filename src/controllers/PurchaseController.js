...
exports.createPurchase = async (req, res) => {
  ...
  try {
    ...
    // XÁC ĐỊNH KHO THỰC TẾ (Chỉ người có quyền mới được chọn kho trong danh sách cho phép)
    const activeWarehouseId = warehouse_id || req.user.warehouse_id;
    ...
    for (const item of items) {
      if (
        ...
        throw new Error(
          `Xe ${item.engine_no || "chưa rõ Số máy"} đang thiếu thông tin quan trọng. Vui lòng kiểm tra lại tất cả các trường!`,
        );
      }
      ...
      if (existing) {
        throw new Error(
          `Số máy ${item.engine_no} hoặc Số khung ${item.chassis_no} đã tồn tại trong hệ thống (Tình trạng: ${existing.status}). Không thể nhập trùng!`,
        );
      }

      if (Number(item.price_vnd) < 0)
        throw new Error("Giá nhập hàng của xe không được âm!");
...
exports.addPayment = async (req, res) => {
  ...
  try {
    ...
    if (
      req.user.role !== "ADMIN" &&
      !req.user.allowedWarehouses.includes(purchase.warehouse_id)
    ) {
      throw new Error(
        "Bạn không có quyền thanh toán cho đơn hàng thuộc kho này!",
      );
    }

    const payment = await PurchasePayment.create(
      { purchase_id, amount_paid_vnd, payment_date, notes },
      { transaction },
    );
    purchase.paid_amount_vnd =
      Number(purchase.paid_amount_vnd || 0) + Number(amount_paid_vnd);
...
exports.deleteLot = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      throw new Error("Bạn không có quyền xóa lô hàng của kho này!");
    }
    ...
    if ((soldVehicles.length > 0 || transferItems.length > 0) && !force) {
      ...
      if (soldVehicles.length > 0)
        reasons.push(`${soldVehicles.length} xe đã bán`);
      if (transferItems.length > 0)
        reasons.push(`${transferItems.length} lượt chuyển kho`);
    ...
    for (const v of soldVehicles) {
      if (v.retail_sale_id) {
        ...
        await RetailPayment.destroy({
          where: { retail_sale_id: v.retail_sale_id },
          transaction,
        });
        await RetailSale.destroy({
          where: { id: v.retail_sale_id },
          transaction,
        });
      }
      if (v.wholesale_sale_id) {
        ...
        await WholesalePayment.destroy({
          where: { wholesale_sale_id: v.wholesale_sale_id },
          transaction,
        });
        await WholesaleSale.destroy({
          where: { id: v.wholesale_sale_id },
          transaction,
        });
      }
    }
    ...
    if (transferItems.length > 0) {
      const transferIds = [
        ...new Set(transferItems.map((ti) => ti.transfer_id)),
      ];
      ...
      for (const tId of transferIds) {
        ...
        if (transfer) {
          ...
          const allItemsInTransfer = await TransferItem.findAll({
          ...
          await TransferPayment.destroy({
            where: { transfer_id: tId },
            transaction,
          });
          await TransferLog.destroy({
            where: { transfer_id: tId },
            transaction,
          });
    ...
    res.json({
      message:
        "Đã xóa toàn bộ lô hàng và tất cả giao dịch liên quan thành công!",
    });
  } catch (error) {
...
exports.deleteVehicleFromPurchase = async (req, res) => {
  ...
  try {
    ...
    const vehicle = await Vehicle.findOne({
      where: { id: vehicle_id, purchase_id },
      transaction,
    });
    ...
    if (vehicle.status === "Sold" && force) {
      if (vehicle.retail_sale_id) {
        ...
        const RetailPayment = require("../models/RetailPayment");
        await RetailPayment.destroy({
          where: { retail_sale_id: vehicle.retail_sale_id },
          transaction,
        });
        await RetailSale.destroy({
          where: { id: vehicle.retail_sale_id },
          transaction,
        });
      }
      if (vehicle.wholesale_sale_id) {
        ...
        const WholesalePayment = require("../models/WholesalePayment");
        await WholesalePayment.destroy({
          where: { wholesale_sale_id: vehicle.wholesale_sale_id },
          transaction,
        });
        await WholesaleSale.destroy({
          where: { id: vehicle.wholesale_sale_id },
          transaction,
        });
      }
    }
    ...
    purchase.total_amount_vnd = Math.max(
      0,
      Number(purchase.total_amount_vnd || 0) - vehiclePrice,
    );
    await purchase.save({ transaction });
...
exports.deletePayment = async (req, res) => {
  ...
  try {
    ...
    const purchase = await Purchase.findByPk(payment.purchase_id, {
      transaction,
    });
    ...
    purchase.paid_amount_vnd = Math.max(
      0,
      Number(purchase.paid_amount_vnd || 0) - Number(payment.amount_paid_vnd),
    );
    await purchase.save({ transaction });
...
exports.bulkFixCodes = async (req, res) => {
  ...
  try {
    ...
    for (const vehicle of vehicles) {
      const vType = await VehicleType.findByPk(vehicle.type_id, {
        transaction,
      });
      if (!vType) continue;
      ...
      if (
        vType.chassis_prefix &&
        !newChassis.startsWith(vType.chassis_prefix)
      ) {
      ...
      if (updated) {
        ...
        if (existing)
          throw new Error(
            `Không thể sửa Số máy ${vehicle.engine_no} -> ${newEngine} vì mã mới đã tồn tại trên hệ thống.`,
          );
...
exports.payAllPurchases = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { supplier_id, amount_paid_vnd, payment_date, notes } = req.body;
    ...
    if (unpaidPurchases.length === 0)
      throw new Error(
        "Không có nợ cần thanh toán cho nhà cung cấp này trong phạm vi kho được phép!",
      );
    ...
    for (const purchase of unpaidPurchases) {
      ...
      const debt =
        Number(purchase.total_amount_vnd) - Number(purchase.paid_amount_vnd);
      ...
      const payment = await PurchasePayment.create(
        {
          ...
          payment_date,
          notes: notes ? `${notes} (Thanh toán gộp lô)` : "Thanh toán gộp lô",
        },
        { transaction },
      );

      purchase.paid_amount_vnd =
        Number(purchase.paid_amount_vnd) + paymentForThisLot;
      await purchase.save({ transaction });
    ...
    await transaction.commit();
    res.status(201).json({
      message: `Đã thanh toán gộp cho ${payments.length} lô hàng. Còn thừa: ${remainingAmount.toLocaleString()} đ`,
      payments,
